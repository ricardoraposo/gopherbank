...
func (nq *NotificationQuery) QueryAccount() *AccountQuery {
...
func (nq *NotificationQuery) Clone() *NotificationQuery {
	...
	return &NotificationQuery{
		...
		withAccount: nq.withAccount.Clone(),
...
func (nq *NotificationQuery) WithAccount(opts ...func(*AccountQuery)) *NotificationQuery {
	query := (&AccountClient{config: nq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	nq.withAccount = query
	return nq
}
...
func (nq *NotificationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Notification, error) {
	...
	if query := nq.withAccount; query != nil {
		if err := nq.loadAccount(ctx, query, nodes, nil,
			func(n *Notification, e *Account) { n.Edges.Account = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (nq *NotificationQuery) loadAccount(ctx context.Context, query *AccountQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *Account)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Notification)
	for i := range nodes {
		if nodes[i].account_notification == nil {
			continue
		}
		fk := *nodes[i].account_notification
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "account_notification" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
